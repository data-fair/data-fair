...
export const useSettingsStore = (type: string, id: string) => {
  const settingsFetch = useFetch<Settings>($apiPath + '/settings/' + type + '/' + id)
  ...
  const patch = useAsyncAction(async (patch: Partial<Settings>) => {
    const updatedSettings = await $fetch<Settings>('/settings/' + type + '/' + id, {
      method: 'PATCH',
      body: patch
    })
