import { Resource, type Permission } from '#api/types'

export default {
  isInDepartmentPermission (p: Permission, resource: Resource) {
    return (!p.department || (!resource.owner.department && p.department === '-') || p.department === resource.owner.department)
  },
  isContribWriteAllPermission (p: Permission, resource: Resource) {
