  ...
  <v-app :dark="env.theme.dark">
    <v-toolbar app scroll-off-screen :color="(user && user.adminMode) ? 'admin' : 'default'" :dark="env.theme.dark || (user && user.adminMode)" :class="env.theme.dark || (user && user.adminMode) ? 'main-toolbar' : 'main-toolbar main-toolbar-light'" height="64">
      ...
      <v-toolbar-title>
        ...
        </h1><small>{{ env.brand.description }}</small>
      </v-toolbar-title>
      ...
      <v-toolbar-items class="hidden-md-and-down">
        <v-btn :to="localePath('index')" flat color="primary" exact>
          Accueil
        </v-btn>
        <v-btn :to="localePath({name: 'datasets', query: searchQuery('datasets')})" :class="routePrefix === 'dataset' ? 'v-btn--active' : ''" flat color="primary">
          Jeux de données
        </v-btn>
        <v-btn :to="localePath({name: 'applications', query: searchQuery('applications')})" :class="routePrefix === 'application' ? 'v-btn--active' : ''" flat color="primary">
          Applications
        </v-btn>
        <v-btn :to="localePath({name: 'catalogs', query: searchQuery('catalogs')})" :class="routePrefix === 'catalog' ? 'v-btn--active' : ''" flat color="primary">
          Catalogues
        </v-btn>
        <v-btn :to="localePath({name: 'remote-services', query: searchQuery('remote-services')})" :class="routePrefix === 'remote' ? 'v-btn--active' : ''" flat color="primary">
          Services
        </v-btn>
        <v-menu>
          <v-btn slot="activator" :class="(routePrefix === 'user' || routePrefix === 'interoperate') ? 'v-btn--active' : ''" flat>
            Documentation
          </v-btn>
          <v-list>
