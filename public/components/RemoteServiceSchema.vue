<template lang="html">
  ...
  >
    <template slot="items" slot-scope="props">
      <tr v-if="vocabulary">
        <td>
          <v-icon v-if="!props.item.inputCollection || !props.item.outputCollection" title="Opération unitaire">
            description
          </v-icon>
          <v-icon v-if="props.item.inputCollection && props.item.outputCollection" title="Opération de masse">
            view_list
          </v-icon>
        </td>
        ...
        <td>
          ...
          <template v-for="input in props.item.input">
            <v-chip v-if="vocabulary[input.concept]" :key="input.concept" :title="vocabulary[input.concept].description" style="margin:4px 4px;">
