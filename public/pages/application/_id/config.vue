      ...
      <v-layout row wrap>
        <v-flex xs12 md4>
          ...
          <v-form v-if="showForm" v-model="formValid" @submit="e => {writeConfig(editConfig); e.preventDefault()}">
            <v-jsonschema-form :schema="schema" :model="editConfig" :options="{disableAll: !can('writeConfig'), context: {owner: application.owner}}" @error="error => eventBus.$emit('notification', {error})" />
            <v-layout row>
              <v-spacer/>
