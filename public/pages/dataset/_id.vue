  ...
  <v-layout v-if="dataset" column class="dataset">
    <v-navigation-drawer app fixed stateless :permanent="mini || $vuetify.breakpoint.lgAndUp" :temporary="!mini && !$vuetify.breakpoint.lgAndUp" :mini-variant="mini" :value="true">
      <v-list dense>
        ...
        <v-list-tile v-if="!!dataset.schema.find(f => f['x-refersTo'] === 'https://schema.org/startDate')" :disabled="!can('readLines')" :nuxt="true" :to="`/dataset/${dataset.id}/calendar`">
    ...
    <div class="actions-buttons">
      <v-menu bottom left>
        ...
        <v-list>
          ...
          <v-list-tile :href="downloadFullLink" :disabled="!can('downloadFullData') || (!dataset.isRest && (!dataset.extensions || !dataset.extensions.find(e => e.active)))">
            <v-list-tile-avatar>
              <v-icon color="primary">
                file_download
              </v-icon>
