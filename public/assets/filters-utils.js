...
export function readQueryParams (query, dataset) {
  ...
  Object.keys(query).filter(key => key.startsWith('_c_')).forEach(key => {
    const conceptId = key.replace('_c_', '')
    const conceptField = dataset.schema.find(p => p['x-concept'] && p['x-concept'].primary && p['x-concept'].id === conceptId)
    filters.push({
      hidden: true,
      type: 'in',
      field: conceptField,
