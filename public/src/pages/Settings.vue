<template>
  <md-layout md-align="center">
    <md-layout md-column md-flex="90" v-if="settings">
        ...
        <md-table v-if="operations && operations.length">
          <md-table-header>
            <md-table-row>
              ...
              <md-table-head v-for="role in organizationRoles" :key="role">{{ role }}</md-table-head>
            </md-table-row>
          </md-table-header>
          ...
          <md-table-body>
            <md-table-row v-for="(operation, rowIndex) in Object.values(operations)" :key="rowIndex">
              ...
              <md-table-cell v-for="role in organizationRoles" :key="role">
                <md-checkbox :md-value="role" v-model="settings.operationsPermissions[operation.id]" @change="save"/>
              </md-table-cell>
            </md-table-row>
          </md-table-body>
        </md-table>
      ...
      <licenses :settings="settings" @license-updated="save"/>

      <h3 class="md-headline">Webhooks</h3>
      <webhooks :settings="settings" @webhook-updated="save"/>
    </md-layout>
  </md-layout>
</template>
