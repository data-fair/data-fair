<template>
  <div>
    ...
    <md-list v-if="settings">
      <md-list-item v-for="(webhook, rowIndex) in settings.webhooks" :key="rowIndex" style="padding:8px 0">
        <md-card style="padding:16px;width:100%">
          <md-layout md-row md-vertical-align="center">
            ...
            <md-layout md-flex="50" md-column>
              ...
              <ul>
                <li v-for="(event, i) in webhook.events" :key="i">{{ events[event].text }}</li>
              </ul>
            </md-layout>
    ...
    <md-dialog md-open-from="#new-webhook" md-close-to="#new-webhook" ref="new-webhook-dialog">
      ...
      <md-dialog-content>
        <md-input-container>
          <label>Titre</label>
          <md-input v-model="newWebhook.title"/>
        </md-input-container>

        <md-input-container>
          <label>URL</label>
          <md-input v-model="newWebhook.url"/>
        </md-input-container>

        <md-layout md-column>
          <md-subheader>Evènements</md-subheader>
          <md-checkbox :md-value="eventId" v-model="newWebhook.events" v-for="(event, eventId) in events" :key="eventId">{{ event.text }}</md-checkbox>
        </md-layout>
      </md-dialog-content>
