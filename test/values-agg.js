...
describe('values aggs', () => {
  it('Get values buckets', async () => {
    ...
    res = await ax.get('/api/v1/datasets/dataset/values/id?q=kou&q_mode=complete')
    assert.equal(res.data.length, 1)
    assert.equal(res.data[0], 'koumoul')
    res = await ax.get('/api/v1/datasets/dataset/values/somedate')
    assert.equal(res.data.length, 2)
    assert.equal(res.data[0], '2017-10-10T00:00:00.000Z')
    res = await ax.get('/api/v1/datasets/dataset/values/somedate?q=2017-10&q_mode=complete')
    assert.equal(res.data[0], '2017-10-10T00:00:00.000Z')
  })
})
