...
describe('field sniffer', () => {
  it('Work with booleans', () => {
    ...
    assert.equal(sniffer.format('True', 'boolean'), true)
    assert.equal(sniffer.format('1', 'boolean'), true)
    assert.equal(sniffer.format('-1', 'boolean'), false)
    assert.equal(sniffer.format('vrai', 'boolean'), true)
    assert.equal(sniffer.format('faux', 'boolean'), false)
    assert.equal(sniffer.format('oui', 'boolean'), true)
    assert.equal(sniffer.format('non', 'boolean'), false)
    assert.equal(sniffer.format('yes', 'boolean'), true)
    assert.equal(sniffer.format('no', 'boolean'), false)
  })
  ...
  it('Work with numbers', () => {
    ...
    assert.equal(sniffer.format('-11', 'number'), -11)
    assert.equal(sniffer.format('-1', 'integer'), -1)
    assert.equal(sniffer.format('10 426', 'integer'), 10426)
    assert.equal(sniffer.format('20 000.2', 'number'), 20000.2)
    assert.equal(sniffer.format('27 589', 'integer'), 27589)
    assert.equal(sniffer.format('10,10', 'number'), 10.1)
