...
describe('datasets', function () {
  ...
  it('Upload dataset and update with different file name', async function () {
    ...
    assert.deepEqual(await fs.readdir('../data/test/user/dmeadus0/datasets/dataset-name/data-files/'), ['dataset-name.csv'])
    ...
    assert.ok(res.data.store_bytes.consumption > 150)
    assert.ok(res.data.store_bytes.consumption < 300)
    assert.deepEqual(await fs.readdir('../data/test/user/dmeadus0/datasets/dataset-name/data-files/'), ['dataset-name2.csv'])
  })
