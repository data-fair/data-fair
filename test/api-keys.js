...
describe.only('API keys', () => {
  it('Reject wrong api key', async () => {
    const ax = await ax.builder(null, { headers: { 'x-apiKey': 'wrong' } })
    try {
  ...
  it.only('Create and use a User level api key', async () => {
    const t = new Date().getTime()

    const res = await ax.dmeadus.put('/api/v1/settings/user/dmeadus0', {
      apiKeys: [
    ...
    // Right scope
    const axKey1 = await ax.builder(null, { headers: { 'x-apiKey': key1 } })
    await axKey1.get('/api/v1/stats')
    console.log(new Date().getTime() - t)

    // Wrong scope
    const axKey2 = await ax.builder(null, { headers: { 'x-apiKey': key2 } })
    try {
  ...
  it('Create and use an organization level api key', async () => {
    const res = await ax.cdurning2.put('/api/v1/settings/organization/3sSi7xDIK', {
      apiKeys: [
