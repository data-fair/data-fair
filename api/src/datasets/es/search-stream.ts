...
// Mirror the buffered `search` request but with `asStream: true`, returning a streamed `LinesSource`.
// Same `prepareQuery`, same querystring (`allow_partial_search_results:'false'` + `timeout`), same
// abort/timeout accounting. Under `asStream` the client hands back the response body as a Node stream and
// does NOT auto-decompress, so we gunzip when the response is gzip-encoded (we let the client negotiate
// encoding exactly like the buffered path). Vector-tile (`vtXYZ`) requests are a hard/collect format and
// never use this streamed path, so their script_fields shaping is intentionally omitted here.
export async function searchStream (client: Client, dataset: any, query: any, abortContext?: EsAbortContext): Promise<LinesSource> {
  ...
  const decoded: Readable = encoding.includes('gzip') ? body.pipe(createGunzip()) : body
