...
export const deleteLine = async (req: RequestWithRestDataset & { params: { lineId: string } }, res: Response, next: NextFunction) => {
  ...
  emitLinesUpdated(dataset, reqSessionAuthenticated(req), { nbDeleted: 1 })
}
...
export const createOrUpdateLine = async (req: RequestWithRestDataset, res: Response, next: NextFunction) => {
  ...
  if (_action === 'delete') {
    ...
    finalStatus = operation._status || (definedId ? 200 : 201)
  ...
  if (finalStatus === 204) emitLinesUpdated(dataset, sessionState, { nbDeleted: 1 })
  else if (finalStatus === 201) emitLinesUpdated(dataset, sessionState, { nbCreated: 1 })
  else if (finalStatus === 200) emitLinesUpdated(dataset, sessionState, { nbModified: 1 })
}
...
export const patchLine = async (req: RequestWithRestDataset, res: Response, next: NextFunction) => {
  ...
  if (operation._status !== 304) emitLinesUpdated(dataset, reqSessionAuthenticated(req), { nbModified: 1 })
}
...
export const deleteAllLines = async (req: RequestWithRestDataset, res: Response, next: NextFunction) => {
  ...
  emitLinesUpdated(dataset, reqSessionAuthenticated(req), { deletedAll: true })
}
...
export const bulkLines = async (req: RequestWithRestDataset & { files?: { attachments?: ReqFile[], actions?: ReqFile[] } }, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!summary.cancelled && !streamErrored) {
      emitLinesUpdated(dataset, reqSessionAuthenticated(req), {
