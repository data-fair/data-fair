      ...
      >
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            color="primary"
            :loading="saving"
            :disabled="!formValid || duplicateName ||!editValue.name"
            @click="save"
          />
        </v-card-actions>
      </v-form>
...
export default {
  ...
  computed: {
    ...mapState('application', ['application', 'lineUploadProgress']),
    schema () {
      return applicationSchema.properties.attachments.items
    },
    duplicateName () {
      if (!this.editValue.name) return false
      return !!(this.application.attachments || []).find((a, i) => i !== this.index && a.name === this.editValue.name)
    }
  },
  methods: {
    ...mapActions('application', ['patchAndApplyRemoteChange']),
    onToggle () {
    ...
    async save () {
      ...
      try {
        ...
        await this.$axios.$post('api/v1/applications/' + this.application.id + '/attachments', formData, options)

        if (this.value.name && this.editValue.name !== this.value.name) {
          await this.$axios.$delete('api/v1/applications/' + this.application.id + '/attachments/' + this.value.name)
        }
