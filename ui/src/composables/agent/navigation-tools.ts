  ...
  useAgentTool({
    ...
    execute: async () => {
      ...
      pagesByGroup.push(
        ...
        '- Dataset table: /dataset/{id}/table — accepts filter query params (same format as search_data filters: column_key + suffix like nom_search, age_lte, ville_eq). Also accepts _c_q (full-text search), sort, select, _c_bbox, _c_geo_distance, _c_date_match. The `_c_` prefix on q/bbox/geo_distance/date_match is required for URL sync — never put `_c_` on column filters (use nom_search, not _c_nom_search). Use the filterQuery from dataset_data subagent Context directly as the query parameter.\n' +
