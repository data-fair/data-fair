...
import { startApiServer, stopApiServer, scratchData, checkPendingTasks, dmeadus, formHeaders, sendDataset } from './utils/index.ts'
...
describe('workers', function () {
  ...
  it('Process newly uploaded CSV dataset', async function () {
    ...
    const idField = dataset.schema.find((f: any) => f.key === 'id')
    const dateField = dataset.schema.find((f: any) => f.key === 'some_date')
    ...
    const esUtils = await import('../api/src/datasets/es/index.ts')
    const esIndices = await es.client.indices.get({ index: esUtils.aliasName(dataset) })
    const esIndex = Object.values(esIndices)[0] as any
    ...
    const esIndex2 = Object.values(esIndices2)[0] as any
