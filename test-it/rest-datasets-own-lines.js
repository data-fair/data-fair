...
describe('REST datasets with owner specific lines', function () {
  ...
  it('Create empty REST dataset with activated line ownership', async function () {
    ...
    await assert.rejects(dmeadusOrg.get(`/api/v1/datasets/${dataset.id}/own/user:cdurning2/lines`), (err) => err.status === 403)

    // external user cannot do anything yet
    await assert.rejects(cdurning2.get(`/api/v1/datasets/${dataset.id}/lines`), (err) => err.status === 403)
    await assert.rejects(cdurning2.get(`/api/v1/datasets/${dataset.id}/own/user:cdurning2/lines`), (err) => err.status === 403)
    ...
    assert.rejects(alone.get(`/api/v1/datasets/${dataset.id}/own/user:cdurning2/lines`), (err) => err.status === 403)
    assert.rejects(alone.get(`/api/v1/datasets/${dataset.id}/own/user:alone/lines/cdurningline`), (err) => err.status === 404)
    ...
    await workers.hook('finalize/' + dataset.id)
    assert.rejects(alone.get(`/api/v1/datasets/${dataset.id}/own/user:alone/lines/aloneline`), (err) => err.status === 404)
    ...
    await assert.rejects(cdurning2.get(`/api/v1/datasets/${dataset.id}/schema`), (err) => err.status === 403)
  })
