...
import { startApiServer, stopApiServer, scratchData, checkPendingTasks, dmeadus, anonymous, config } from './utils/index.ts'
...
describe('Cache headers', function () {
  ...
  const createDataset = async (ax: any) => {
    let dataset = (await ax.post('/api/v1/datasets', {
      isRest: true,
      title: 'cache-headers-' + Math.random().toString(36).substring(7),
