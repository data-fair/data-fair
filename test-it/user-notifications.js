...
describe('user-notifications about dataset', function () {
  ...
  it('send user notification', async function () {
    ...
    await assert.rejects(ngernier4Org.post(`/api/v1/datasets/${dataset.id}/user-notification`, { topic: 'topic1', title: 'Title' }), (err) => err.status === 403)
    await ax.put(`/api/v1/datasets/${dataset.id}/permissions`, [
    ...
    await assert.rejects(ngernier4Org.post(`/api/v1/datasets/${dataset.id}/user-notification`, { topic: 'topic1', title: 'Title', visibility: 'public' }), (err) => err.status === 403)
