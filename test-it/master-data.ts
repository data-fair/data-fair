...
import { startApiServer, stopApiServer, scratchData, checkPendingTasks, superadmin, dmeadus, cdurning2, formHeaders, sendDataset } from './utils/index.ts'
...
`
  ...
  it('manage query syntax errors', async function () {
    ...
    await assert.rejects(workers.hook('finalize/slave'), (err: any) => {
  ...
  it('return multiple levels of extended properties', async function () {
    ...
    await workers.hook('finalize/master1')
    ...
    await workers.hook('finalize/master2')
  ...
  it('should prevent using master-data without permission on dataset', async function () {
    ...
    await superadmin.patch('/api/v1/remote-services/' + remoteService.id, { public: true })
