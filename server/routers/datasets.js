...
function clean(dataset, thumbnail = '300x200', draft = false, publicUrl) {
...
router.get('', cacheHeaders.noCache, asyncWrap(async(req, res) => {
  ...
  results.forEach(r => {
    ...
    clean(r, req.query.thumbnail, false, req.publicBaseUrl)
  })
...
router.get('/:datasetId', readDataset(), applicationKey, permissions.middleware('readDescription', 'read'), cacheHeaders.noCache, (req, res, next) => {
  ...
  res.status(200).send(clean(req.dataset, req.query.thumbnail, false, req.publicBaseUrl))
})
...
router.patch('/:datasetId', readDataset((patch) => {
  ...
  if (Object.keys(patch).length === 0) return res.status(200).send(clean(req.dataset, null, false, req.publicBaseUrl))
  ...
  res.status(200).json(clean(req.datasetnull, false, req.publicBaseUrl))
}))
...
router.put('/:datasetId/owner', readDataset(), permissions.middleware('delete', 'admin'), asyncWrap(async(req, res) => {
  ...
  res.status(200).json(clean(patchedDataset, null, false, req.publicBaseUrl))
}))
...
router.post('', beforeUpload, checkStorage(true), filesUtils.uploadFile(), filesUtils.fixFormBody(validatePost), asyncWrap(async(req, res) => {
  ...
  try {
    ...
    res.status(201).send(clean(dataset, null, req.query.draft === 'true', req.publicBaseUrl))
  } catch (err) {
...
const updateDataset = asyncWrap(async(req, res) => {
  ...
  try {
    ...
    await syncRemoteService(db, dataset)
    res.status(req.isNewDataset ? 201 : 200).send(clean(dataset, null, req.query.draft === 'true', req.publicBaseUrl))
  } catch (err) {
    // Wrapped the whole thing in a try/catch to remove files in case of failure
    for (const file of req.files) {
      await fs.remove(file.path)
