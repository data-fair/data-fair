...
exports.deleteLine = async (req, res, next) => {
  ...
  await db.collection('datasets').updateOne({ id: req.dataset.id }, { $set: { _restPartialUpdate: 'index' } })
    ...
    try {
      ...
      if (drop) {
        if (summary.nbErrors) {
          ...
          await db.collection('datasets').updateOne({ id: req.dataset.id }, { $set: { status: 'updated', _currentUpdate: { reExtend: true } } })
        }
      } else {
        await db.collection('datasets').updateOne({ id: req.dataset.id }, { $set: { _restPartialUpdate: 'index' } })
      }
...
exports.applyTTL = async (app, dataset) => {
  ...
  if (summary.nbOk) patch._restPartialUpdate = 'index'
