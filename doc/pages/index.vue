  ...
  <div>
      ...
      <v-container fill-height>
        <v-layout align-center>
          <v-flex text-xs-center>
            ...
            <span class="headline">{{ $t("common.description") }}</span>
    ...
    <v-container justify-center>
      <v-flex md10 offset-md1>
        <blockquote class="blockquote display-1" style="text-align: center">
          ...
          <v-card height="100%">
            <v-card-title primary-title style="height:15%">
              <h3 class="title" style="text-align: center">
                {{ $t(`pages.user-guide.${feature.id}.title`) }}
              </h3>
            </v-card-title>
            <v-img :src="feature.img" height="160px" />
            <v-card-text style="height:45%;min-height:80px">
              {{ $t(`pages.user-guide.${feature.id}.description`) }}
            </v-card-text>

            <v-card-actions style="height:10%">
              <v-spacer />
              <v-btn :to="localePath({name: 'user-guide-id', params: {id: feature.id}})" color="primary">
                {{ $t("common.seemore") }}
              </v-btn>
            </v-card-actions>
          </v-card>
