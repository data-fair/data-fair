...
import { startApiServer, stopApiServer, scratchData, checkPendingTasks, dmeadus, sendDataset, formHeaders, cdurning2, hlalonde3, hlalonde3Org, ngernier4 } from './utils/index.ts'
...
describe('virtual datasets', function () {
  ...
  it('Compatibility is accepted for integer/number types', async function () {
    ...
    const workers = await import('../api/src/workers/index.ts')
    datasetInt = await workers.hook('finalize/' + datasetInt.id)
  ...
  it('Apply static filter', async function () {
    ...
    const newVirtualDataset = await workers.hook('finalize/' + virtualDataset.id)
  ...
  it('Add another virtual dataset as child', async function () {
    ...
    const virtualDataset2 = await workers.hook('finalize/' + res.data.id)
    ...
    const virtualDataset3 = await workers.hook('finalize/' + res.data.id)
  ...
  it('A virtual dataset cannot have a child virtual dataset with filters (no way to enforce them)', async function () {
    ...
    await assert.rejects(workers.hook('finalize/' + virtualDataset.id), (err: any) => {
      console.log(err)
      return true
    })
  ...
  it('A virtual dataset is updated after a child schema changes', async function () {
    ...
    virtualDateField = await workers.hook('finalize/' + virtualDataset.id)
  ...
  it('A virtual dataset has the most restrictive capabilities of its children', async function () {
    ...
    const workers = await import('../api/src/workers/index.ts')
    await workers.hook('finalize/' + child1.id)
    ...
    await workers.hook('finalize/' + child2.id)
    ...
    let virtualDataset = await workers.hook('finalize/' + res.data.id)
  ...
  it('A virtual dataset of a geo parent can serve tiles', async function () {
    ...
    await workers.hook('finalize/' + virtual1.id)
    ...
    await workers.hook('finalize/' + virtual2.id)
  ...
  it('a virtual dataset of a dataset with attachments re-expose those attachments', async function () {
    ...
    const workers = await import('../api/src/workers/index.ts')
    await workers.hook('finalize/' + child.id)
