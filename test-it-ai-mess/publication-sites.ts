...
import validateDcat from '@data-fair/data-fair-api/src/misc/utils/dcat/validate.js'
import { startApiServer, stopApiServer, scratchData, checkPendingTasks, dmeadusOrg, dmeadus, hlalonde3Org, ddecruce5Org, ngernier4Org } from './utils/index.ts'
...
describe('publication sites', function () {
  ...
  it('should publish dataset on a org site and access it from re-exposition of data-fair', async function () {
    ...
    await assert.rejects(ax.get(`http://localhost:5601/data-fair/api/v1/datasets/${dataset.id}`), (err: any) => {
      ...
      assert.equal(err.response?.data || err.data, 'publication site unknown')
      return true
    })
    ...
    await assert.rejects(ax.get(`http://localhost:5601/data-fair/api/v1/datasets/${otherDataset.id}`), (err: any) => {
      assert.equal(err.response?.status || err.status, 404)
      assert.equal(err.response?.data || err.data, 'Dataset not found')
      return true
    })
  ...
  it('department admin can request publishing dataset on org site', async function () {
    let notif: any
    testEvents.on('notification', (n: any) => {
      notif = n
    })
  ...
  it('department contrib can request publishing dataset on department site', async function () {
    let notif: any
    testEvents.on('notification', (n: any) => { notif = n })
