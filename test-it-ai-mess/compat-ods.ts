...
import { startApiServer, stopApiServer, scratchData, checkPendingTasks, dmeadusOrg, sendDataset } from './utils/index.ts'
...
describe('compatibility layer for ods api', function () {
  ...
  it('exposes records and exports api on 2 urls', async function () {
    ...
    await assert.rejects(ax.get(`/api/v1/datasets/${dataset.id}/compat-ods/records`, { params: { where: 'id: koumoul' } }), { status: 400 })
    ...
    res = await ax.get(`/api/v1/datasets/${dataset.id}/compat-ods/records`, { params: { group_by: 'id', select: 'count(*) as count' } })
    assert.deepEqual(res.data.results, [{ id: 'bidule', count: 1 }, { id: 'koumoul', count: 1 }])
