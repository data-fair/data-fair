...
describe('Datasets with auto-initialization from another one', function () {
  ...
  it('Create REST dataset with copied information from file dataset', async function () {
    ...
    attachmentForm.append('attachment', fs.readFileSync(path.resolve(import.meta.dirname, '../test-it/resources/avatar.jpeg')), 'avatar.jpeg')
...
2024-01-13,2024-01-13T19:42:02.790Z,13/01/2024,13/01/2024 19:42`, { headers: { 'content-type': 'text/csv' } })
  ...
  it('Create REST and file datasets with copied information from virtual dataset', async function () {
    ...
    const checkDatasetAttachments = async (dataset: any) => {
      ...
      const odtItem = lines.results.find((item: any) => item.attachment === 'test.odt')
      assert.ok(odtItem)
...
,"no attachment on this line","2020-01-13"`)
  ...
  it('Create file dataset that doesn\'t match imported schema', async function () {
    ...
    const form = new FormData()
    form.append('file', fs.readFileSync(path.resolve(import.meta.dirname, '../test-it/resources/datasets/dataset2.csv')), 'dataset2.csv')
    ...
    assert.ok(event.data.startsWith('La structure du fichier contient des ruptures de compatibilit'))
    event = journal.pop()
    assert.equal(event.draft, true)
  ...
  it('Prevent initializing a dataset when missing permissions', async function () {
    const { ngernier4 } = await import('./utils/index.ts')
    const workers = await import('../api/src/workers/index.ts')
  ...
  it('Initialize dataset in a department from dataset in orga', async function () {
    const { dmeadusOrg } = await import('./utils/index.ts')
    const ax = dmeadusOrg
