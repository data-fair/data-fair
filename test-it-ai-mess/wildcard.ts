...
describe('Wildcard fields', function () {
  ...
  it('Search in dataset using a contains query on a wildcard field', async function () {
    ...
    let res = await ax.post('/api/v1/datasets/wildcards/_bulk_lines', Object.keys(items).map((key: any) => ({ _id: key, content: items[key] })))
  ...
  it('Wildcards can be activated after first indexation', async function () {
    ...
    await assert.rejects(ax.get('/api/v1/datasets/wildcards2/lines', { params: { qs: 'content.wildcard:*suite' } }), { status: 400 })
