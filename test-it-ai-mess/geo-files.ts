...
describe('geo files support', function () {
  ...
  it('Process uploaded GPX dataset', async function () {
    ...
    const oldLimit = { ...(config as any).defaultLimits }
    ;(config as any).defaultLimits.totalStorage = (config as any).defaultLimits.datasetStorage = 10000000
    ...
    ;(config as any).defaultLimits = oldLimit
