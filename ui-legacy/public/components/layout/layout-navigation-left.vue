  ...
  computed: {
    ...
    navigation () {
      ...
      if (this.canAdminDep && this.env.portalsIntegration) {
        contentGroup.items.push({ to: '/pages', icon: 'mdi-view-dashboard-edit', title: 'Pages de portails' })
        contentGroup.items.push({ to: '/reuses', icon: 'mdi-page-next', title: 'Réutilisations' })
      }
      ...
      if (this.canAdminDep && this.env.portalsIntegration) {
        managementGroup.items.push({ to: '/portals', icon: 'mdi-monitor-dashboard', title: 'Portails' })
      }
