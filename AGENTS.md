...
- [Dev environment resource limits](docs/architecture/dev-resources.md) — why the vite dev server and Elasticsearch are memory-capped: running several dev stacks side by side can exhaust host RAM and swap, and the resulting global OOM kill destroys the desktop session while leaving the containers running
...
- [Parent resources (`partOf`)](docs/architecture/part-of.md) — the child annotation on datasets and applications: the generic model and its link table in `shared`, definition-time rules, the `writePartOf` gate, default listing filter, the lifecycle cascades (`childrenAction`, owner change, virtual member detach and last-member guards), integrity classification and known limits. **Read before touching `partOf` or the virtual-dataset membership guards.**
