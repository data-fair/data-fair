...
- [Streamed-source `/lines`](docs/architecture/streamed-read-lines.md) — the as-built read path: `LinesSource` abstraction, hits-splitter + envelope skeleton, the assemble-then-`res.send` model (stream the source, not the response — so ETag/Link/304 are all preserved, zero observable change), the pbf/shp zero-copy raw-buffer-to-worker paths, error/backpressure, parity contract, staging go/no-go.
- [Read hot-path optimization](docs/architecture/read-hot-path-optimization.md) — the **why** behind the above: the GC/RSS symptom, why streamed *output* was rejected (Link/ETag), the investigation arc + verdicts (parser bake-off, Rust, streaming experiment, threshold sweep, compiled serializer — all in `benchmark/results/`), the per-format routing decisions (incl. why xlsx stays buffered), what's shelved, and the measurement lessons. **Read before touching the `/lines` read path.**
