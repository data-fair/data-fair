...
- [Permissions recap](docs/architecture/permissions-recap.md) — the account-wide "what can this kind of visitor reach?" screen: the effective-access model, `scopeFilter` and its three deliberate differences from `filterCan`, the three security locks, and the scope URL contract shared with a future portals iframe. **Read before touching permission filtering or the scope parameters.**
