...
2024-01-13,2024-01-13T19:42:02.790Z,13/01/2024,13/01/2024 19:42`, { headers: { 'content-type': 'text/csv' } })
    await waitForFinalize(ax, initFromDataset.id)
  ...
  test('Create REST and file datasets with copied information from virtual dataset', async () => {
    ...
    const initFromDatasetRest = await waitForFinalize(ax, res.data.id)
    ...
    const initFromDatasetFile = await waitForFinalize(ax, res.data.id)
...
,"no attachment on this line","2020-01-13"`)
  ...
  test('Create draft file dataset with copied information from another file dataset', async () => {
    ...
    initFromDataset = await waitForFinalize(ax, res.data.id)
  ...
  test('Prevent initializing a dataset when missing permissions', async () => {
    ...
    assert.ok(errorEvent.data.includes('permission manquante'))
  })
