...
test.describe('values aggs', () => {
  ...
  test('Get values buckets', async () => {
    ...
    assert.deepEqual(dataset.schema.find((p: any) => p.key === 'somedate').enum, ['2017-12-12', '2017-10-10'])
    ...
    await assert.rejects(ax.get(`/api/v1/datasets/${dataset.id}/values_agg?field=unknown`), (err: any) => {
      assert.equal(err.status, 400)
      assert.ok(err.data.includes('Le paramètre "field" référence un champ inconnu unknown'))
      return true
    })
    const newSchema = [...dataset.schema]
    const adrProp = newSchema.find((p: any) => p.key === 'adr')
    adrProp['x-capabilities'] = { values: false }
    await doAndWaitForFinalize(ax, dataset.id, () => ax.patch(`/api/v1/datasets/${dataset.id}`, { schema: newSchema }))
    await assert.rejects(ax.get(`/api/v1/datasets/${dataset.id}/values_agg?field=adr`), (err: any) => {
      assert.equal(err.status, 400)
      assert.ok(err.data.includes('Impossible de grouper sur le champ adr'))
      return true
    })
    await assert.rejects(ax.get(`/api/v1/datasets/${dataset.id}/values_agg?field=id&sort=adr`), (err: any) => {
      assert.equal(err.status, 400)
  ...
  test('performs calculations on a field', async () => {
    ...
    await waitForFinalize(ax, 'metric-agg')
