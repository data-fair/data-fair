...
test.describe('shared permission operations / picker', () => {
  test('REST dataset exposes line + admin + own-line operations, never deprecated ones', () => {
    ...
    assert.deepEqual(picker.admin.map((o) => o.id).sort(), ['changeOwner', 'delete', 'getPermissions', 'setPermissions', 'setReadApiKey', 'writeExports', 'writePublicationSites', 'writePublications'].sort())
