...
test.describe('API keys', () => {
  ...
  test('Reject wrong api key', async () => {
    const ax = axios({ headers: { 'x-apiKey': 'wrong' } })
  ...
  test('Create and use a User level api key', async () => {
    ...
    // Right scope
    const axKey1 = axios({ headers: { 'x-apiKey': key1.clearKey } })
    ...
    const axKey5 = axios({ headers: { 'x-apiKey': key5.clearKey } })
    await assert.rejects(sendDataset('datasets/dataset1.csv', axKey5), { status: 403 })

    // expired key
    const axKey3 = axios({ headers: { 'x-apiKey': key3.clearKey } })
  ...
  test('Create and use a department level api key', async () => {
    ...
    // Set the correct owner
    const axKey1 = axios({ headers: { 'x-apiKey': key1 } })
    const dataset = await sendDataset('datasets/dataset1.csv', axKey1)
    assert.equal(dataset.status, 'finalized')
    assert.equal(dataset.owner.type, 'organization')
    assert.equal(dataset.owner.id, 'KWqAGZ4mG')
