...
test('admin actions answer 409 while the per-dataset worker lock is held', async () => {
  ...
  const ack = (await anonymousAx.post(`${apiUrl}/api/v1/test-env/lock/datasets:${dataset.id}`)).data.ack
  ...
  try {
    ...
    await anonymousAx.delete(`${apiUrl}/api/v1/test-env/lock/datasets:${dataset.id}`)
  }
...
test('historized storage is metered into the owner consumption, deleted-dataset tail included', async () => {
  ...
  const measured = (await anonymousAx.post(`${apiUrl}/api/v1/test-env/integrity-storage/run`)).data
  ...
  await anonymousAx.post(`${apiUrl}/api/v1/test-env/integrity-storage/run`)
