...
- **(RESOLVED 2026-06-16)** `query-advice.ts` importing `permissions.ts` (config-coupled) broke its unit spec under the `unit` project (no config dir → `config.ts` import-time `assertValid` throws). Fix: moved the cross-cutting accessors (`resource`/`resourceType`/`bypassPermissions`/`publicOperation`) from `permissions.ts` into the config-free `req-context.ts`; `permissions.ts` re-exports them as a facade; config-free consumers (query-advice et al.) import directly from `req-context.ts`. 405 unit tests pass; ratchet unchanged (1354). Convention §2 updated with the config-free-home rule. (The earlier note here was wrong — relocating accessors to a config-free module IS the right fix, not lazy config validation.)
