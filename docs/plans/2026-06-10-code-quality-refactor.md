...
- Cross-cutting properties (read by permissions, cache-headers, …) get accessors in `misc/utils/req-context.ts`: `resource`, `resourceType`, `bypassPermissions`, `publicOperation`, `noCache`, `publicBaseUrl`, `publicWsBaseUrl`, `publicationSite`, `mainPublicationSite`.
...
- Convert the cross-cutting **readers** to req-context accessors: `permissions.ts` (reads `resource`, `resourceType`, `bypassPermissions`; sets `publicOperation` → `setReqPublicOperation`), `cache-headers.js` → `.ts` (reads `noCache`, `resource`, `publicOperation`), `capture.ts`, `application-key.ts` (sets `bypassPermissions`, `matchingApplicationKey`), `api-key.ts` (sets `bypassPermissions`).
