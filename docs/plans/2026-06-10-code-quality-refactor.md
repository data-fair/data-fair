...
| 6c | `refactor-datasets-service` | service.js→ts, middlewares.js→ts, dataset/datasetFull accessors | M | med | actual **−55** (DONE 2026-06-17) |
| 6d | `refactor-datasets-router` | router.js → thin adapters split by sub-resource | XL | high | actual **−350** (891→541), `datasets/` 100% TS, DONE 2026-06-18 |
...
### Phase 6c execution record (2026-06-17, branch `refactor-typescript8`)
Sub-plan: `docs/plans/2026-06-17-refactor-datasets-service.md`. Executed in the **main session** (per 6b lesson #2 — subagents can't get `dangerouslyDisableSandbox`, so they can't run ratchet/eslint/tests).
- **Result:** `datasets/service.js` + `datasets/middlewares.js` → `.ts`. Ratchet **946 → 891 (−55)**; baseline committed. **0 new suppressions** (and the old `@ts-ignore`s in `middlewares.js` + `/** @type {any} */` JSDoc casts in `service.js` removed); lint clean; 442 unit specs pass. `service.ts` was already express-free → pure type annotations, bodies verbatim.
- **Accessors:** new **module-local** `dataset`/`datasetFull` (`setReqDataset`/`reqDataset`/`reqDatasetOptional` + `…DatasetFull`, `legacyProp` dual-write). `resource`/`noCache` set via the existing `setReqResource`/`setReqNoCache`; `publicationSite`/`mainPublicationSite`/`resource` read via existing optional getters. Module-local home is safe — the pure/unit-tested `query-advice.ts` and `datasets/utils/*` readers consume the raw `legacyProp`, not the accessor (no config-free module imports `middlewares.ts`). `req.url = withQuery(...)` kept (genuine Express prop). `legacyProp` for `dataset`/`datasetFull` (and `esAbortContext`) **retained** — `router.js` (≈213 reads) + `datasets/utils/*` still raw-read; drop in 6d.
- **webhooks.trigger typing fix (in passing, no behavior change):** its `event` param was the journal `Event` (requires `date`) but it only reads `type`/`href`/`data`, and callers pass webhook-shaped `{type, body?}`; `sender` was untyped+unused. Widened to `{type, href?, data?, body?}` / `sender?: any` — fixes the `{type}`-vs-`Event` + arg-count errors at the source; all callers still type-check.
- **Boot check WITHOUT infra (worktree services down):** ran a Node `--experimental-strip-types` import-resolution probe on `app.js`, `datasets/router.js` and every cross-module importer — all resolve their full transitive graph (only config validation throws; **no `ERR_MODULE_NOT_FOUND`**). This is the rename boot-check the 6a/6b lesson demands; api/e2e specs deferred to husky pre-push (infra is user-managed).

### Phase 6d execution record (2026-06-18, branch `refactor-typescript9`)
Sub-plan: `docs/plans/2026-06-17-refactor-datasets-router.md` (full record there). Executed in the **main session**, **11 commits**, one PR (user-managed). `api/src/datasets/router.js` (1569 LOC, 342 errors, 12 `@ts-ignore`) → 9 `routes/*.ts` thin-adapter groups (+ `_common.ts`/`_es-error.ts` shared helpers) composed by a **33-line `router.ts`**. `datasets/` is now **100% TypeScript**.
- **Result:** ratchet **891 → 541 (−350)**; baseline committed each commit. **All 12 `@ts-ignore`s gone**, no new suppressions; `as any` only for documented preserved-bugs / dynamic payloads. Mount + middleware-chain order preserved exactly (metadata → write → lines → own-lines → master-data → read → files → misc, after the global `setReqResourceType` `.use`). Each commit ran ≥1 api spec; final sweep ran the whole `tests/features/datasets` tree.
- **Accessors:** added `linesOwner` + `_draft` (module-local in `middlewares.ts`, `legacyProp` — rest.ts/upload.ts raw-read) and `operation` (`permissions.ts`, no legacyProp — atomic). **Dropped now-dead legacyProps** `resource`/`datasetFull`/`esAbortContext` (grep-verified). **Kept** `dataset` (query-advice/upload/rest/outputs) + `resourceType` (ods raw-sets).
- **Systemic source-typing fixes** (recurring `router.<verb>` overload friction from custom-`req`-typed middlewares, each verified accessor-only): `permissions.middleware`/`canDoForOwnerMiddleware`/`application-key` default → `req: Request`; `clamav.middleware` `Response` imported from express (resolves the parking-lot DOM-Response item); `res.throttle`/`throttleEnd` declared on global `Express.Response`; `permissions.middleware` `trackingCategory?: string | null`; `tiles.geojson2pbf` `vtPrepared?`; `outputs.results2sheet` `bookType='xlsx'` + `ReqWithDataset` exported; `preparePatch` `draftValidationMode?`; config `cache.disabled?` schema.
- **4 suspected bugs preserved bit-for-bit** → parking lot items 10–13 below.

### Phase 7 execution record (2026-06-18, branch `typescript-refactor10`) — FINAL PHASE
Sub-plan: `docs/plans/2026-06-18-refactor-api-compat.md`. Executed in the **main session** (one PR, user-managed). `api-compat/ods/index.ts` was already TS, so the work was (A) extract the pure ODS→ES query translation + result shaping into `api-compat/ods/operations.ts` (verbatim bodies: `prepareEsQuery`/`parseFilters`/`completeSort`/`isoWithOffset`/`prepareResult`/`transforms`/`applyAliases`/`sortBuckets`/`prepareBucketResult` + `compatReqCounter`/`logCompatODSError`); index.ts keeps routing / ES calls / streaming / response sending. New pinning unit spec `tests/features/datasets/compat/compat-ods-operations.unit.spec.ts` (28 cases).
- **Config-free gotcha (the documented pitfall):** `operations.ts` imports `getFilterableFields` from the **config-free** `datasets/es/operations.ts`, NOT the `es/index.ts` barrel (which loads `#config`), so the unit spec doesn't crash under the `unit` project (no config dir). Same rule that bit `query-advice` post-Phase-5.
- **(B) Migrated api-compat's last raw mutations:** `req.resourceType = 'datasets'` (×2) → `setReqResourceType`; `req.noModifiedCache = true` → `setReqNoModifiedCache`; `(req as any).dataset` reads → `reqDataset`. Removed 2 `@ts-ignore`.
- **(C) Dropped BOTH remaining `legacyProp`s** (`resourceType` + `dataset`), three-grep gate verified empty. `dataset` accessor moved `datasets/middlewares.ts` → **config-free `req-context.ts`** (re-exported as a facade so the 8 `routes/*` importers are untouched); added `reqRestDataset` (RestDataset, single contained cast). This unblocked migrating `query-advice.ts` (config-free), `datasets/utils/rest.ts` (~40 reads, no longer needs the would-be require cycle), `outputs.ts`, `upload.ts`. Removed `resourceType` from `RequestWithResource`. `fakeReq` in the query-advice unit spec now sets the dataset via `setReqDataset` (fixture change; assertions unchanged).
- **Out of scope (documented follow-ups):** `linesOwner` + `_draft` legacyProps remain (`rest.ts` / `upload.ts` raw-read them). No new suspected bugs found (all preserved).
- **Result:** ratchet **541 → 540** (api-compat 51→50 across index+operations; net −1 after the dataset/resourceType cleanup); baseline committed. Lint clean, no new suppressions. **Verified:** 58 unit specs (compat-ods + operations + query-advice), api specs compat-ods (9), rest crud/bulk/history/own-lines (51), attachments (18) — all green; dev-api booted on the edits (no require cycle / `ERR_MODULE_NOT_FOUND`). **The express-decoupling refactor series (Phases 0–7) is complete.**
...
10. **datasets PATCH never passes a draft validation mode** (found in Phase 6d, 2026-06-18, preserved bit-for-bit; `datasets/routes/metadata.ts` PATCH handler, ex `router.js` patch): `preparePatch(req.app, patch, dataset, sessionState, locale)` is called with **5 args**, omitting the 6th `draftValidationMode` (PUT/`updateDatasetRoute` passes it; metadata PATCH does not). `preparePatch`'s param was made optional to compile; so a PATCH-created/updated draft always uses the default/undefined validation mode. Verify intent + add a test, fix in a dedicated PR.

11. **cancelDraft passes a stray arg to journals.log** (found in Phase 6d, preserved bit-for-bit; `datasets/routes/write.ts` draft DELETE, ex `router.js`): the call was `journals.log('datasets', dataset, {type:'draft-cancelled'}, false, sessionState)` — a **5th `sessionState` arg** to the 4-param `journals.log(resourceType, resource, event, noStoreEvent)`, silently ignored at runtime. Dropped as a behavior-preserving no-op. If the draft-cancelled journal event *should* carry session context, that's the fix (+ a 5th param on `journals.log`).

12. **draft-validated notification localizedParams is mis-shaped** (found in Phase 6d, preserved bit-for-bit; `datasets/routes/write.ts` validateDraft, ex `router.js`): `localizedParams: { cause: { fr, en } }` (param→locale) instead of the `{ fr: { cause }, en: { cause } }` (locale→param) shape every other call site uses (e.g. `workers/batch-processor/process-file.ts:275`, the `Record<Locale, Record<string,string>>` type). So the localized "cause" is malformed in the draft-validated notification. Preserved via cast; fix the shape + a test.

13. **user-notification passes `true` as the notification sessionState** (found in Phase 6d, preserved bit-for-bit; `datasets/routes/misc.ts` user-notification POST, ex `router.js`): `notifications.send(notif, true, sessionState)` against the current 2-param `send(event, sessionState?)` — so `true` lands in the `sessionState` position and the real `sessionState` is the ignored 3rd arg. Likely a leftover from an old 3-param signature. Preserved as `send(notif, true as any)`; verify the intended sessionState is wired + a test.
