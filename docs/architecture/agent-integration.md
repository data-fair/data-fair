...
| **The one-turn lag** | `editLine_form` registers with the dialog, so it is absent from the tool list of the very request whose tool call opened it. The openers used to say "you can now delegate"; a judged run took that literally, found no such subagent, and spent a junk `data_quality_checker` dispatch (`{"task":"test"}`) to reach the turn where it existed — leaving the person an apology about an internal tool activating and a stray audit chip in the chat. They now carry `navigate`'s wording: finish the reply, it is there next turn. |
