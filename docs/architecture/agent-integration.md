...
| **The one-turn lag** | `editLine_form` registers with the dialog, so it is absent from the tool list of the very request whose call opened it. Neither obvious wording works: "you can now delegate" is false for that request — a judged run took it literally and burned a turn on a junk `data_quality_checker` dispatch to reach one where it was true — and `navigate`'s "finish your reply, it is there next turn" **deadlocked three runs**, because the person was waiting to be told a button was ready, so no next turn ever came and the conversation drained to the cap. The dialog now emits `dataset-line-dialog-opened`, and the results tell the model to declare `wait_for_user_action`: a pending unkeyed transition resolves it at once, the turn continues mid-flight, and the rebuilt tool list has the subagent in it. **Every result that mentions this lag must give the same answer** — the next run after the fix still had `navigate` carrying "finish your reply" two calls earlier, and the model obeyed that one at the add dialog while obeying the new one at the edit dialog. `navigate` now points at the same wait (arriving resolves it: `location` is wait-resolving), and `tests/features/agent-tools/one-turn-lag-advice.unit.spec.ts` pins that they cannot drift apart. |
...
| `dataset-line-dialog-opened` | `ui/src/components/dataset/table/dataset-table.vue` | `id`, `mode` (`add` / `edit`) — emitted when a line dialog opens, so a declared wait resolves and the same turn continues with `editLine_form` in scope |
