...
**`_c_` prefix is concept-only.** The `_c_` prefix is reserved for concept filters (geo distance, date match, full-text search) that are identified by concept id rather than column key. A column filter must never carry this prefix — `ville_eq` is correct, `_c_ville_eq` is wrong. A `_c_`-prefixed column filter resolves against concept ids, finds nothing, and is silently dropped, so the query runs unfiltered with no error. The `filtersGuide` in `agent-tools/_utils.ts` (embedded once in the `dataset_data` subagent prompt), the terse `filtersDescription` stub on each filtering tool's `filters` param, and the `navigate` tool all state this explicitly so the model never produces that pattern. The main system prompt (`ui/src/layouts/default.vue`) must stay consistent: it tells the orchestrator to reuse the subagent's `filterQuery` *verbatim, without altering its prefixes*, and never to add `_c_` to a column filter. (It once wrongly described `filterQuery` as "already _c_-prefixed", which made the orchestrator instruct the subagent to slap `_c_` onto column filters in the presentation layer — past the API's `ignoredParamsAdvice` hint, since the corrupted prefix only ever appeared in the emitted link, never in an actual query.)
