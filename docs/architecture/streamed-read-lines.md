...
**Total serializer.** `prepareResultItem` is designed never to throw on data; it falls back gracefully. A residual mid-stream exception calls `res.destroy(err)`. The enveloped JSON format (opening `{` + `"results":[` … `]}`) makes truncation detectable by the client: an unterminated body causes a JSON parse error rather than silent data loss.
