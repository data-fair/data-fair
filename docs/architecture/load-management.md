...
- `_exists` / `_nexists` — **mapping-driven, and the only suffixes with no capability gate** (`FILTER_CAPABILITIES` maps them to `null`). `resolveExistsFields` returns every indexed representation of the column that can answer existence, and the filter is refused with a **400** when there is none. Elasticsearch answers `exists` from the inverted index, from `doc_values`, or from `_field_names`, so the main field is usable as long as `index` **or** `values` is on — un-flagged columns therefore keep the single fast keyword `exists` query. When both are off (how `prepareSchema` configures a `textarea` / `markdown` column: `{index: false, values: false, insensitive: false}`) the main field is in none of the three and an `exists` on it silently matches nothing, so it is left out entirely and existence is answered by the analyzed sub-fields instead — `.wildcard` if configured, else the union of `.text_standard` and `.text`, plus `.keyword_insensitive` on an un-flagged column (an exact, token-free signal, but capped by the same `ignore_above`). Flagged columns whose main field IS usable union it with the same analyzed sub-fields, so that long values — which always produce at least one analyzed token — are also found. For a flagged pure-keyword column (no `.text` / `.text_standard` sub-field) the union fallback is keyword-only, which cannot cover dropped long values; that case is surfaced by the `queryAdviceUncertainFilter` per-request hint instead. The table header menu mirrors the same rule in `useHeaderFilters.showExists`.
