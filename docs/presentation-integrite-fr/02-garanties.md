...
| **Écriture directe dans les données servies** (accès à l'index de recherche par lequel les lecteurs consultent le jeu de données, en contournant la plateforme) | Altérer, insérer ou supprimer des données vues par les lecteurs, sans toucher aux sources vérifiées | **Détection** au contrôle nocturne (comptage systématique et fenêtres d'échantillonnage aléatoires), **détection exhaustive** à la demande, **réparation** par réindexation depuis la source vérifiée — la preuve de la divergence est conservée avant que la réparation ne l'efface |
...
- **imputation individuelle bornée dans le temps, pas permanente** : quand l'écriture est authentifiée (session utilisateur ou clé d'API), l'identité de l'auteur — identifiant, adresse IP, pays — est bien **capturée et scellée**, mais à part de la révision, dans une **attribution à rétention propre** (180 jours, jamais prolongée — voir la section conformité). La révision scellée, elle, n'enregistre que la *catégorie* d'auteur (utilisateur, superadministrateur, traitement interne…), jamais l'identité : un choix délibéré lié à la protection des données personnelles, parce que le verrou d'une révision peut glisser indéfiniment tant que la ressource est vivante. Passée la fenêtre d'attribution, seul le journal d'activité de la plateforme porte le « qui », avec son propre cycle de vie ;
